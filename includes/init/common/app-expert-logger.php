...
class App_Expert_Logger
{
    ...
    public static function createLogFile()
    {
        ...
        // ========================== Edits to match wp requirement ==========================
        $plugin_log_path = self::get_log_dir_path();
        static::$log_file = $plugin_log_path . "/log-{$time}.log";


        //Check if directory /logs exists
        if (!file_exists($plugin_log_path)) {
            mkdir($plugin_log_path, 0777, true);
        }
...
//        fwrite(static::$file, "{$timeLog}{$pathLog}{$lineLog}: {$severityLog} - {$messageLog} {$contextLog}" . PHP_EOL);
        fwrite(static::$file, "{$timeLog}{$lineLog}: {$severityLog} - {$messageLog} {$contextLog}" . PHP_EOL);
